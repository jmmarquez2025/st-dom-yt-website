...
| I7 | n/a | Third parties | **Praise.** Cloudflare Web Analytics only loads if `VITE_CLOUDFLARE_WEB_ANALYTICS_TOKEN` is set. YouTube embed (`YouTubeChannel.jsx:145`) gated behind user click. Vatican News widget (`VaticanNews.jsx:6,34`) gated behind user click. No Google Maps, no gtag, no Facebook pixel. Privacy + perf posture is right. |
